...
# ---------------------------------------------------------------------------
def test_todos_open_two_examples():
    raw = _collection(
        [
...
def test_todos_completed_two_examples():
    raw = _collection(
        [
...
def test_todos_deferred_two_examples():
    raw = _collection(
        [
...
def test_concerns_anxieties_two_examples():
    raw = _collection(
        [
...
def test_concerns_fears_two_examples():
    raw = _collection(
        [
...
def test_successes_achievements_two_examples():
    raw = _collection(
        [
...
def test_successes_milestones_two_examples():
    raw = _collection(
        [
...
def test_successes_positive_moments_two_examples():
    raw = _collection(
        [
...
def test_general_reflections_two_examples():
...
def test_general_observations_two_examples():
    raw = _collection(
        [
...
# Invariants
# ---------------------------------------------------------------------------
def test_no_entry_is_dropped_with_empty_columns():
...
def test_by_date_index_mirrors_fields():
    raw = _collection(
        [
...
def test_metadata_reflects_input():
    raw = _collection(
        [
...
def test_duplicate_sentences_in_same_entry_are_deduped():
    raw = _collection([_entry(eid=1, cue="Bugün başardım.", notes="Bugün başardım.")])
    out = ParserService().parse(raw)
    assert len(out.fields.successes.achievements) == 1


def test_parse_rejects_wrong_input_type():
    service = ParserService()
    with pytest.raises(ParserError):
        service.parse({"not": "a collection"})  # type: ignore[arg-type]


def test_empty_collection_produces_empty_tree():
